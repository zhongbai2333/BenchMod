...
public final class ClientScenarioRegistry {
    ...
    public void registerProviders(List<BenchProvider> providers) {
        for (BenchProvider provider : providers) {
            ...
            if (!(provider instanceof BenchClientProvider clientProvider)) {
                throw new IllegalStateException("Provider " + provider.id() + " has no integrated-client scenarios");
            }
            clientProvider.registerClient((descriptor, factory) -> register(provider.id(), descriptor, factory));
        }
        if (registrations.isEmpty()) {
            throw new IllegalStateException("No integrated-client scenarios were registered");
        }
    }

    public List<Registration> registrations() { return List.copyOf(registrations); }

    private void register(String providerId, ScenarioDescriptor descriptor, BenchClientScenarioFactory factory) {
    ...
    public record Registration(
            String providerId, ScenarioDescriptor descriptor, BenchClientScenarioFactory factory) {}
}